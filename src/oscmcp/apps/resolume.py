...
class ResolumeArena:
    ...
    async def start(self) -> None:
        ...
        logger.info(f"Resolume Arena OSC interface started. Listening on port {self.listen_port}, sending to port {self.target_port}")
    ...
    def play_composition(self, composition: int) -> None:
        ...
        """
        self.send(f"/composition/selected", composition - 1)
    ...
    def set_effect_parameter(self, layer: int, effect: int, parameter: int, value: float) -> None:
        ...
        """
        self.send(f"/composition/layers/{layer}/effects/{effect}/{parameter}/value", float(value))
    ...
    def toggle_effect_bypass(self, layer: int, effect: int, bypassed: bool = None) -> None:
        ...
        if bypassed is None:
            ...
            self.send(f"/composition/layers/{layer}/effects/{effect}/bypass", 1 if bypassed else 0)
