...
class SuperColliderOSC:
    ...
    async def start(self) -> None:
        ...
        logger.info(f"SuperCollider OSC interface started. Listening on port {self.listen_port}, sending to port {self.target_port}")
    ...
    def s_new(self, defname: str, node_id: int = -1, add_action: int = 0, target: int = 1, **args) -> None:
...
async def example_usage():
    ...
    def on_status(status):
        print(f"SuperCollider status: {status['synth_count']} synths, "
