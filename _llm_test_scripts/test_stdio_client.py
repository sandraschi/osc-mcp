        ...
        while True:
            ...
            for line in lines[:-1]:  # Process all complete lines
                ...
                if line.startswith(b"|") or not (line.startswith(b"{") or line.startswith(b"[")):
                    logger.debug(
                        f"Skipping non-JSON line: {line.decode('utf-8', errors='replace')}"
                    )
                    continue
                ...
                try:
                    ...
                    logger.debug(
                        f"Failed to parse JSON from line: {line.decode('utf-8', errors='replace')}"
                    )
                    continue
...
async def test_osc_functionality():
    ...
    try:
        ...
        listener_result = await client.call_method(
            "start_osc_listener", {"port": 9000, "address": "127.0.0.1"}
        )
        logger.info(f"OSC listener started: {json.dumps(listener_result, indent=2)}")

        # Test 3: Send OSC message
        logger.info("\n--- Sending test OSC message ---")
        send_result = await client.call_method(
            "send_osc_message",
            {
                "host": "127.0.0.1",
                "port": 9000,
