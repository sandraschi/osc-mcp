...
async def test_vca_gating(
    vco_module_id=1, vco_param_id=0, vca_module_id=None, vca_param_id=None, port=10001
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--vca-module", type=int, default=None, help="VCA module ID (optional, for VCA gating)"
    )
    parser.add_argument(
        "--vca-param", type=int, default=None, help="VCA level parameter ID (optional)"
    )
    parser.add_argument("--port", type=int, default=10001, help="OSC port (default: 10001)")
    parser.add_argument(
        "--method",
        choices=["vca", "frequency"],
        default=None,
    ...
    if args.method == "vca" or (args.vca_module is not None):
        ...
        asyncio.run(
            test_frequency_gating(
                vco_module_id=args.vco_module, vco_param_id=args.vco_param, port=args.port
            )
        )
