...
fn install_dir_from_backend(path: &PathBuf) -> PathBuf {
    if let Some(parent) = path.parent() {
        if parent.file_name().is_some_and(|name| name.eq_ignore_ascii_case("resources")) {
...
fn free_port(port: u16) -> bool {
    #[cfg(windows)]
    {
        ...
        let img_kill = format!(
            "Stop-Process -Name 'osc-mcp-backend' -Force -ErrorAction SilentlyContinue; \
             Stop-Process -Name 'osc-mcp-native' -Force -ErrorAction SilentlyContinue; \
             taskkill /F /IM osc-mcp-backend.exe /T 2>$null; \
             taskkill /F /IM osc-mcp-native.exe /T 2>$null"
        );
            ...
            if i == 15 {
                let elevated = format!(
                    "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                     '-NoProfile -Command \"Stop-Process -Name osc-mcp-backend -Force -ErrorAction SilentlyContinue; \
                     taskkill /F /IM osc-mcp-backend.exe /T 2>$null; \
                     Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                     ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
                );
...
pub fn spawn_backend(app: AppHandle, state: &BackendProcess) -> Result<String, String> {
    ...
    log_line(&app, &format!("spawning {} (cwd {}) on port {BACKEND_PORT}",
