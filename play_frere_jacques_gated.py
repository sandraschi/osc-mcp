...
FRERE_JACQUES_FREQUENCIES = [
    ...
    261.63, 196.00, 261.63,           # C4 G3 C4
    261.63, 196.00, 261.63,           # C4 G3 C4
]

# Note durations (in beats)
DURATIONS = [
    0.5, 0.5, 0.5, 0.5,  # C D E C
    0.5, 0.5, 0.5, 0.5,  # C D E C
    0.5, 0.5, 1.0,        # E F G (long)
    0.5, 0.5, 1.0,        # E F G (long)
    0.25, 0.25, 0.25, 0.25, 0.5, 0.5,  # G A G F E C
    0.25, 0.25, 0.25, 0.25, 0.5, 0.5,  # G A G F E C
    0.5, 0.5, 1.0,        # C G C (long)
    0.5, 0.5, 1.0,        # C G C (long)
]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Play Frère Jacques with note gating"
    )
    ...
    parser.add_argument("--vca-module", type=int, default=None,
                       help="VCA module ID (optional, for gating)")
    parser.add_argument("--vca-param", type=int, default=0,
                       help="VCA level parameter ID (default: 0)")
    parser.add_argument("--port", type=int, default=10001,
                       help="OSC port (default: 10001)")
    parser.add_argument("--tempo", type=int, default=120,
                       help="Tempo in BPM (default: 120)")
    parser.add_argument("--no-gating", action="store_true",
                       help="Disable gating (just rapid frequency changes)")
    args = parser.parse_args()
    
    asyncio.run(play_frere_jacques_gated(
