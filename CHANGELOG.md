...
### Added
- **`supercollider_manager`**: added `create_group`/`free_group` (`/g_new`/`/g_freeAll`, real and documented, previously unexposed) and `status` - a real, bidirectional `/status` <-> `/status.reply` query, the one genuinely two-way operation across every app-manager in this fleet besides QLab/obs-websocket. Implemented via a dedicated request/reply UDP socket (`_query_scsynth_status`, run in a thread), not the shared fire-and-forget `send_osc` helper. Live-verified against a real running `scsynth.exe` 3.14.1: real synth/group/ugen/CPU/sample-rate telemetry, including genuine sample-rate clock drift (48020.65 Hz actual vs 48000 Hz nominal) that couldn't be faked. Rebuilt `web_sota/src/pages/supercollider.tsx` with tabs (Synth Control / Groups / Server Status / Setup), the Server Status tab showing this real telemetry with a Refresh button instead of a static placeholder.
...
### Fixed
- **`web_sota/src/pages/supercollider.tsx`**: the "Set Node Controls" amp/freq sliders called a fictional `set_node_parameter` operation with a `parameter` kwarg - the real operation is `set_control` with `control_name`, predates this session's changes. Also fixed a `data.content`-vs-`data.result.content` response-unwrapping bug shared with `ableton.tsx`/`maxmsp.tsx`/`touchdesigner.tsx`/`vrchat.tsx` (cosmetic-only there - it only made the Command Output panel show the raw double-wrapped envelope instead of the clean result - but blocking here, since the new Server Status tab needs the unwrapped fields).
- **`vcvrack.tsx`**: "Play"/"Stop" called `vcv_manager("start_transport"/"stop_transport")` and "Setup Organ Patch" called `vcv_manager("setup_organ")` - none of these are real operations, and the first two are structurally impossible (VCV Rack has no global transport concept; OSCelot has no broadcast address). Removed, replaced with a real preset-download depot (new `api/v1/endpoints/vcv_presets.py`) and the tool's actual `set_parameter`/`trigger`/`sync_reaper_tempo` operations, which had no UI at all before.
- **`ableton_manager`**'s webapp page: "Record" and "Scene Launch" called `record`/`launch_scene`, operations that never existed server-side at all. Added real `trigger_record`/`fire_scene` operations (`/live/song/trigger_session_record`, `/live/scene/fire`) plus `stop_all_clips`/`tap_tempo`/`stop_clip`/`set_mute`/`set_solo`, and fixed `set_tempo`/`set_volume`/`set_pan`'s stale parameter names left over from the action->operation rename.
