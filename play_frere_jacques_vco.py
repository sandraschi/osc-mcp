...
def frequency_to_note_name(freq):
    ...
    if abs(freq - 261.63) < 5:
        ...
        return "E4"
    elif abs(freq - 349.23) < 5:
        return "F4"
    elif abs(freq - 392.00) < 5:
        return "G4"
    elif abs(freq - 440.00) < 5:
        return "A4"
    elif abs(freq - 196.00) < 5:
        return "G3"
    else:
        return f"{freq:.1f}Hz"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--param-id", type=int, default=0,
                       help="Frequency parameter ID (default: 0)")
    ...
    asyncio.run(play_frere_jacques_vco(
