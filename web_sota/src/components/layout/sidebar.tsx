        ...
        >
            <div className="flex h-16 items-center border-b border-slate-800 px-4">
                <div className="flex items-center gap-2 font-semibold text-slate-100">
                    ...
                    {!collapsed && <span className="animate-in fade-in duration-300">OSC-MCP</span>}
                </div>
            </div>
            ...
            <nav className="flex-1 space-y-1 p-2">
                ...
                <div className="space-y-1 px-2 pt-4">
                    ...
                    {targetItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                <item.icon className={cn("h-5 w-5", !collapsed && "mr-3", isActive && "text-emerald-400")} />
                                {!collapsed && <span>{item.label}</span>}
                                {collapsed && (
                                    <div className="absolute left-full ml-2 hidden rounded bg-slate-800 px-2 py-1 text-xs text-white group-hover:block z-50 whitespace-nowrap">
                                        {item.label}
                ...
                <div className="space-y-1 px-2 pt-4">
                    <Link
                        ...
                        className={cn(
                            "group flex items-center rounded-md px-3 py-2 text-sm font-medium transition-colors hover:bg-slate-800 hover:text-white",
                            location.pathname === '/settings' ? "bg-slate-800 text-white" : "text-slate-400",
                            collapsed ? "justify-center" : "justify-start"
                        )}
                    >
                        <Settings className={cn("h-5 w-5", !collapsed && "mr-3", location.pathname === '/settings' && "text-blue-400")} />
