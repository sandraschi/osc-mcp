...
export function Apps() {
    ...
    const fetchApps = async () => {
        ...
        try {
            ...
            // Fallback mock data
            setApps([
                { name: 'osc-mcp', port: 10766, description: 'OSC and signal routing hub', status: 'running' },
                { name: 'virtualization-mcp', port: 10700, description: 'VM operations', status: 'unknown' },
                { name: 'meta-mcp', port: 10719, description: 'Meta fleet operations', status: 'running' },
            ]);
        } finally {
            setLoading(false);
        }
        ...
        <div className="p-6 space-y-6">
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredApps.map((app) => (
                        <Card key={app.name} className="border-slate-800 bg-slate-950/50 hover:bg-slate-900/80 transition-colors group">
            ...
            {!loading && filteredApps.length === 0 && (
                <div className="flex flex-col items-center justify-center p-12 text-center border border-dashed border-slate-800 rounded-lg bg-slate-950/20">
                    ...
                    <p className="text-sm text-slate-500">Try adjusting your search query or ensure the Fleet Registry is available.</p>
                </div>
            )}
        </div>
    );
