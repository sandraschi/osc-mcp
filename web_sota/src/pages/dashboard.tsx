...
import { Activity, GitMerge, Box, Cpu, Radio, Music, Monitor, Gamepad2, Beaker, Guitar, MessageCircle } from "lucide-react";
...
const TARGET_ICONS: Record<string, React.ElementType> = {
    ableton: Music, touchdesigner: Monitor, vrchat: Gamepad2,
    maxmsp: Radio, supercollider: Beaker, vcvrack: Guitar,
};
...
export function Dashboard() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!cancelled) { setHealth(h); setStats(s); }
            } catch (e) {
    ...
    const onlineTargets = stats ? Object.values(stats.targets).filter(t => t.status === "online" || t.status === "unknown").length : 0;
        ...
        <div className="space-y-6">
            ...
            <div className="relative overflow-hidden rounded-xl border border-slate-800 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-8">
                ...
                <div className="relative z-10">
                    ...
                    <h1 className="text-3xl font-bold text-white mb-2">OSC-MCP Orchestrator</h1>
                    <p className="text-slate-400 max-w-xl text-sm leading-relaxed">
                        Universal OSC bridge with MCP tools, real-time routing, and multi-target orchestration.
                        Route messages between AI agents and any OSC-capable target — DAWs, VJ software, game
                        engines, and modular synthesizers — through a single backend on port 10767.
                    </p>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Card className="border-slate-800 bg-slate-950/50">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium text-slate-200">Available Targets</CardTitle>
                        <Radio className="h-4 w-4 text-emerald-500" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold text-white">{onlineTargets}/{targetCount}</div>
                ...
                <Card className="border-slate-800 bg-slate-950/50">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold text-white">{health?.status === "ok" ? "Connected" : "Offline"}</div>
                        <p className="text-xs text-slate-400">Port {stats?.backend_port ?? 10767}</p>
                    </CardContent>
                </Card>
            </div>
                    ...
                    <CardContent>
                        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                            {Object.entries(stats.targets).map(([name, target]) => {
                                ...
                                const isOnline = target.status === "online" || target.status === "unknown";
                                return (
                                    <div key={name} className="flex items-center gap-3 p-3 rounded-lg border border-slate-800 bg-slate-900/30">
                                        ...
                                        <span className={`h-2 w-2 rounded-full shrink-0 ${isOnline ? "bg-emerald-500" : "bg-red-500"}`} />
                                    </div>
                                );
                            })}
                        </div>
            ...
            {err && (
                <Card className="border-red-800/50 bg-red-950/20">
                    <CardContent className="p-4 text-xs text-red-400">
                        API: {err} — start backend on port 10767 via <code className="text-red-300">uv run python -m oscmcp --http --port 10767</code>
                    </CardContent>
                </Card>
            )}
        </div>
